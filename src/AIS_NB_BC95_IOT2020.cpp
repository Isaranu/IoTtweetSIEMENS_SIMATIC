
#include "AIS_NB_BC95_IOT2020.h"

//################### Buffer #######################
String input;
String buffer;
//################### counter value ################
byte k=0;
//################## flag ##########################
bool end=false;
bool send_NSOMI=false;
bool flag_rcv=true;
//################### Parameter ####################
bool en_rcv=false;
unsigned long previous=0;
unsigned char sendMode = 0;
String sendStr;

void event_null(char *data){}

//AltSoftSerial myserial;

AIS_NB_BC95_IOT2020::AIS_NB_BC95_IOT2020()
{
	Event_debug =  event_null;
}

void AIS_NB_BC95_IOT2020:: setupDevice(String serverPort)
{
		//myserial.begin(9600);
    //_Serial = &myserial;

		Serial1.begin(9600);
		_Serial = &Serial1;

	Serial.println(F("############ AIS_NB_BC95 Modified to compatible with SIEMENS IOT2020 ############"));
	reset();
	String imei = getIMEI();
	if (debug) Serial.print("# Module IMEI-->  ");
	if (debug) Serial.println(imei);
	String fmver = getFirmwareVersion();
	if (debug) Serial.print("# Firmware ver-->  ");
	if (debug) Serial.println(fmver);
	String imsi = getIMSI();
	if (debug) Serial.print("# IMSI SIM-->  ");
	if (debug) Serial.println(imsi);

	attachNB(serverPort);
	createUDPSocket(serverPort);
}

void AIS_NB_BC95_IOT2020:: reset()
{
	rebootModule();
	while (!setPhoneFunction(1))
	{
		Serial.print(F("."));
	}
	Serial.println();
}

void AIS_NB_BC95_IOT2020:: rebootModule()
{
	//delay(1000);

	_Serial->println("AT");
	AIS_NB_BC95_RES res = wait_rx_bc(500,"OK");
	_Serial->println("AT+NRB");
	if (debug) Serial.print("# Reboot Module");
	while (!waitReady());
	{
		if (debug) Serial.print(F("."));
	}
    _Serial->flush();
	delay(5000);
}

bool AIS_NB_BC95_IOT2020:: waitReady()
{
	static bool reset_state=false;
	if(_Serial->available())
	{
		String input = _Serial->readStringUntil('\n');
		if(input.indexOf("OK")!=-1)
		{
				return(true);
		}
	}
	return(false);
}

bool AIS_NB_BC95_IOT2020:: setPhoneFunction(unsigned char mode)
{
	_Serial->print("AT+CFUN=");
	_Serial->println(mode);
	AIS_NB_BC95_RES res = wait_rx_bc(1000,"OK");
	delay(2000);
	return(res.status);
}

String AIS_NB_BC95_IOT2020:: getIMEI()
{
	String out;
	_Serial->println("AT+CGSN=1");
	AIS_NB_BC95_RES res = wait_rx_bc(1000,"OK");
	out = res.temp;
	out.replace("OK","");
	if (out.length() < 7)
	{
		return "";
	}
	out = out.substring(7,out.length());
	res = wait_rx_bc(1000,"OK");
	return (out);
}

String AIS_NB_BC95_IOT2020:: getFirmwareVersion()
{
	_Serial->println("AT+CGMR");
	AIS_NB_BC95_RES res = wait_rx_bc(1000,"OK");
	String out = res.temp;
    //if (debug){Serial.println(out);}
    out.replace("OK","");
	out = out.substring(0,out.length());
	//out = out.substring(10,28);
	res = wait_rx_bc(500,"OK");
	return (out);
}
String AIS_NB_BC95_IOT2020:: getIMSI()
{
	_Serial->println("AT+CIMI");
	AIS_NB_BC95_RES res = wait_rx_bc(1000,"OK");
	String out = res.temp;
    out.replace("OK","");
	out = out.substring(0,out.length());
	res = wait_rx_bc(500,"OK");
	return (out);
}

pingRESP AIS_NB_BC95_IOT2020:: pingIP(String IP)
{
	pingRESP pingr;
	String data = "";
	_Serial->println("AT+NPING=" + IP);

	AIS_NB_BC95_RES res = wait_rx_bc(3000,"+NPING");

	if(res.status)
	{
		data = res.data;
		_Serial->println(data);
		int index = data.indexOf(":");
		int index2 = data.indexOf(",");
		int index3 = data.indexOf(",",index2+1);
		pingr.status = true;
		pingr.addr = data.substring(index+1,index2);
		pingr.ttl = data.substring(index2+1,index3);
		pingr.rtt = data.substring(index3+1,data.length());
		//Serial.println("# Ping Success");
		if (debug) Serial.println("# Ping IP:"+pingr.addr + ",ttl= " + pingr.ttl + ",rtt= " + pingr.rtt);

	}else { if (debug) Serial.println("# Ping Failed");}
	res = wait_rx_bc(500,"OK");
	return pingr;
}

String AIS_NB_BC95_IOT2020:: getDeviceIP()
{
	String data = "";
	_Serial->println("AT+CGPADDR");
	AIS_NB_BC95_RES res = wait_rx_bc(3000,"+CGPADDR");
	if(res.status)
	{
		data = res.data;
		int index = data.indexOf(":");
		int index2 = data.indexOf(",");
		data = res.data.substring(index2+1,data.length());
		if (debug) Serial.println("# Device IP: "+data);

	}else {data = "";}
	res = wait_rx_bc(500,"OK");
	return data;
}

bool AIS_NB_BC95_IOT2020:: setAutoConnectOn()
{
	_Serial->println("AT+NCONFIG=AUTOCONNECT,TRUE");
	AIS_NB_BC95_RES res = wait_rx_bc(1000,"OK");
	return(res.status);
}

bool AIS_NB_BC95_IOT2020:: setAutoConnectOff()
{
	_Serial->println("AT+NCONFIG=AUTOCONNECT,FALSE");
	AIS_NB_BC95_RES res = wait_rx_bc(1000,"OK");
	return(res.status);
}

String AIS_NB_BC95_IOT2020:: getNetworkStatus()
{
	String out = "";
	String data = "";

	_Serial->println("AT+CEREG=2");
	AIS_NB_BC95_RES res = wait_rx_bc(500,"OK");
	_Serial->println("AT+CEREG?");
	 res = wait_rx_bc(2000,"+CEREG");
     if(res.status)
	{
		data = res.data;
		int index = data.indexOf(":");
		int index2 = data.indexOf(",");
		int index3 = data.indexOf(",",index2+1);
		out = data.substring(index2+1,index2+2);
		if (out == "1")
		{
			out = "Registered";
		}else if (out == "0")
		{
			out = "Not Registered";
		}else if (out == "2")
		{
			out = "Trying";
		}
	if (debug) Serial.println("# Get Network Status : " + out);

	}
	res = wait_rx_bc(1000,"OK");
	_Serial->flush();
	return(out);
}
/*
bool AIS_NB_BC95:: setAPN(String apn)
{
	String cmd = "AT+CGDCONT=1,\"IP\",";
	cmd += "\""+apn+"\"";
	_Serial->println(cmd);
	AIS_NB_BC95_RES res = wait_rx_bc(1000,F("OK"));
	return(res.status);
}*/

String AIS_NB_BC95_IOT2020:: getAPN()
{
	String data="";
	String out="";
	_Serial->println("AT+CGDCONT?");
	AIS_NB_BC95_RES res = wait_rx_bc(2000,"+CGDCONT");
	if(res.status)
	{
		int index=0;
		int index2=0;
		data = res.data;
		index = data.indexOf(":");
		index2 = data.indexOf(",");

		index = res.data.indexOf(",",index2+1);
		index2 = res.data.indexOf(",",index+1);
		out = data.substring(index+2,index2-1);
		Serial.println("# Get APN: " + out);
	}
	res = wait_rx_bc(500,"OK");
	return(out);
}

bool AIS_NB_BC95_IOT2020:: attachNB(String serverPort)
{
	bool ret=false;
	if(!getNBConnect())
	{
		if (debug) Serial.print("# Connecting NB-IoT Network");
		for(int i=1;i<60;i+=1)
		{
				setPhoneFunction(1);
				setAutoConnectOn();
				cgatt(1);
				delay(3000);
				if(getNBConnect())
					{
					  ret=true;
					  break;
					}
				Serial.print(".");
		}
	} else
		{
			return true;
		}

	if (ret)
	{
		if (debug) Serial.print("> Connected");
	    createUDPSocket(serverPort);
	}
	else {
			if (debug) Serial.print("> Disconnected");
		 }
	if (debug) Serial.println("\n################################################################");
	return ret;
}
bool AIS_NB_BC95_IOT2020:: detachNB()
{
	bool ret=false;
	_Serial->flush();
	if (debug) Serial.print("# Disconnecting NB-IoT Network");
	cgatt(0);
	delay(1000);
	for(int i=1;i<60;i+=1)
	{
		Serial.print(".");
		if(!getNBConnect())
		{ ret=true; break;}

	}
	if (debug) Serial.println("> Disconnected");
	return ret;
}

bool AIS_NB_BC95_IOT2020:: cgatt(unsigned char mode)
{
	_Serial->print("AT+CGATT=");
	_Serial->println(mode);
	AIS_NB_BC95_RES res = wait_rx_bc(5000,"OK");
	return(res.status);
}

bool AIS_NB_BC95_IOT2020:: getNBConnect()
{
	_Serial->println("AT+CGATT?");
	AIS_NB_BC95_RES res = wait_rx_bc(500,"+CGATT");
	bool ret;
	if(res.status)
	{
        if(res.data.indexOf("+CGATT:0")!=-1)
			ret = false;
		if(res.data.indexOf("+CGATT:1")!=-1)
			ret = true;
	}
	res = wait_rx_bc(500,"OK");
	return(ret);
}

signal AIS_NB_BC95_IOT2020:: getSignal()
{
	_Serial->println("AT+CSQ");
	AIS_NB_BC95_RES res = wait_rx_bc(500,"+CSQ");
	signal sig;
	int x = 0;
	String tmp;
	if(res.status)
	{
		if(res.data.indexOf("+CSQ")!=-1)
		{
			int index = res.data.indexOf(":");
			int index2 = res.data.indexOf(",");
			tmp = res.data.substring(index+1,index2);
			if (tmp == "99")
			{
				sig.csq = "N/A";
				sig.rssi = "N/A";
			}
			else
			{
				sig.csq = tmp;
				x = tmp.toInt();
				x = (2*x)-113;
				sig.rssi = String(x);
			}
			sig.ber  = res.data.substring(index2+1);
			if (debug) Serial.println("# Get CSQ Signal: csq= " + sig.csq + ", rssi= " + sig.rssi + ", ber= " +sig.ber);
		}
	}
	res = wait_rx_bc(500,"OK");
	return(sig);
}

void AIS_NB_BC95_IOT2020:: createUDPSocket(String port)
{
	_Serial->print("AT+NSOCR=DGRAM,17,");
	_Serial->println(port+",1");
	AIS_NB_BC95_RES res = wait_rx_bc(3000,"OK");

	delay(3000);
	res = wait_rx_bc(500,"OK");

}

UDPSend AIS_NB_BC95_IOT2020:: sendUDPmsg( String addressI,String port,unsigned int len,char *data,unsigned char send_mode)
{
	sendMode = send_mode;

	UDPSend ret;
    if(!attachNB(port))
    {
		if (debug) Serial.println("# >Disconnected");
		return ret;
    }
	if (debug) Serial.println("\n################################################################");
	if (debug) Serial.print("# Sending Data IP=");
	if (debug) Serial.print(addressI);
	if (debug) Serial.print(" PORT=");
	if (debug) Serial.print(port);
	if (debug) Serial.println();

	_Serial->print("AT+NSOST=0");
	//if (debug) Serial.print(F("AT+NSOST=0"));
	_Serial->print(",");
	//if (debug) Serial.print(",");
	_Serial->print(addressI);
	//if (debug)Serial.print(addressI);
	_Serial->print(",");
	//if (debug)Serial.print(",");
	_Serial->print(port);
	//if (debug) Serial.print(port);
	_Serial->print(",");
	//if (debug)Serial.print(",");

	if(send_mode == MODE_STRING_HEX)
	{
		_Serial->print(String(len/2));
		//if (debug) Serial.print(String(len/2));
	}
	else
	{
		_Serial->print(String(len));
		//if (debug) Serial.print(String(len));
	}

	_Serial->print(",");

	if (debug) Serial.print("# Data=");
		if(send_mode == MODE_STRING_HEX)
		{
			for(int i=0;i<len;i++)
			{
				_Serial->print(data[i]);
				if (debug) Serial.print(data[i]);
			}
		}
		if(send_mode == MODE_STRING)
		{
			printHEX(data);
		}

	_Serial->println();
	if (debug) Serial.println();

	AIS_NB_BC95_RES res = wait_rx_bc(5000,"OK");
	ret.status = false;
	ret.socket = 0;
	ret.length = 0;
	if(res.status)
	{
	    ret.status = true;
		int index = res.temp.indexOf(",");
		int index2 = res.temp.indexOf("O");
		ret.socket = res.temp.substring(index-1,index).toInt();
		ret.length = res.temp.substring(index+1,index2).toInt();
		if (debug) Serial.println("# Send OK");
	}else {if (debug) Serial.println("# Send ERROR");}

	//Serial.println(F("\n###############################################"));

	return(ret);
}

UDPReceive AIS_NB_BC95_IOT2020:: waitResponse()
{
  unsigned long current=millis();
  UDPReceive rx_ret;

  if(en_rcv && (current-previous>=250) && !(_Serial->available()))
  {
      _Serial->println("AT+NSORF=0,100");
	  //Serial.println(F("AT+NSORF=0,100"));
      previous=current;
  }

  if(_Serial->available())
  {
    char data=(char)_Serial->read();
    if(data=='\n' || data=='\r')
    {
      if(k>2)
      {
        end=true;
        k=0;
      }
      k++;
    }
    else
    {
      input+=data;
    }
    //if(debug) Serial.println(input);
  }
  if(end){
      if(input.indexOf("+NSONMI:")!=-1)
      {
          //if(debug) Serial.print(F("send_NSOMI: "));
          //if(debug) Serial.println(input);
          if(input.indexOf("+NSONMI:")!=-1)
          {
            //if(debug) Serial.print(F("found NSONMI "));
            _Serial->println("AT+NSORF=0,100");
            input="";
            send_NSOMI=true;
          }
          end=false;
      }
      else
        {
          //if(debug) Serial.print(F("get buffer: "));
          //if(debug) Serial.println(input);

          end=false;

            int index1 = input.indexOf(",");
            if(index1!=-1)
            {
              int index2 = input.indexOf(",",index1+1);

			  int index3 = input.indexOf(",",index2+1);
			  int index4 = input.indexOf(",",index3+1);
			  int index5 = input.indexOf(",",index4+1);
			  int index6 = input.indexOf("\r");

			  rx_ret.socket = input.substring(0,index1).toInt();
			  rx_ret.ip_address = input.substring(index1+1,index2);
			  rx_ret.port = input.substring(index2+1,index3).toInt();
			  rx_ret.length = input.substring(index3+1,index4).toInt();
			  rx_ret.data = input.substring(index4+1,index5);
			  rx_ret.remaining_length = input.substring(index5+1,index6).toInt();

			  if (debug) receive_UDP(rx_ret);

           }

          send_NSOMI=false;
          input="";
          }
        }
		return rx_ret;
}//end waitResponse

UDPSend AIS_NB_BC95_IOT2020:: sendUDPmsg(String addressI,String port,String data)
{
	int x_len = data.length();
	char buf[x_len+2];
	data.toCharArray(buf,x_len+1);
	return(sendUDPmsg(addressI,port,x_len,buf,MODE_STRING_HEX));
}
UDPSend AIS_NB_BC95_IOT2020:: sendUDPmsgStr(String addressI,String port,String data)
{
	sendStr = data;
	int x_len = data.length();
	char buf[x_len+2];
	data.toCharArray(buf,x_len+1);
	return(sendUDPmsg(addressI,port,x_len,buf,MODE_STRING));

}

bool AIS_NB_BC95_IOT2020:: closeUDPSocket()
{
	_Serial->println("AT+NSOCL=0");
	AIS_NB_BC95_RES res = wait_rx_bc(1000,"OK");
	return(res.status);
}
AIS_NB_BC95_RES AIS_NB_BC95_IOT2020:: wait_rx_bc(long tout,String str_wait)
{
	unsigned long pv_ok = millis();
	unsigned long current_ok = millis();
	String input;
	unsigned char flag_out=1;
	unsigned char res=-1;
	AIS_NB_BC95_RES res_;
	res_.temp="";
	res_.data = "";

	while(flag_out)
	{
		if(_Serial->available())
		{
			input = _Serial->readStringUntil('\n');

			res_.temp+=input;
			if(input.indexOf(str_wait)!=-1)
			{
				res=1;
				flag_out=0;
			}
		    else if(input.indexOf("ERROR")!=-1)
			{
				res=0;
				flag_out=0;
			}
		}
		current_ok = millis();
		if (current_ok - pv_ok>= tout)
		{
			flag_out=0;
			res=0;
			pv_ok = current_ok;
		}
	}

	res_.status = res;
	res_.data = input;
	return(res_);
}

//Util Function
void AIS_NB_BC95_IOT2020::printHEX(char *str)
{
  char *hstr;
  hstr=str;
  char out[3]="";
  int i=0;
  bool flag=false;
  while(*hstr)
  {
    //flag=itoa((int)*hstr,out,16);
		sprintf(out, "%02X", (int)*hstr);
		flag = true;

    if(flag)
    {
      _Serial->print(out);

      if(debug)
      {
        Serial.print(out);
      }

    }
    hstr++;
  }
}
String AIS_NB_BC95_IOT2020:: toString(String dat)
{
	String str="";
	for(int x=0;x<dat.length();x+=2)
  {
      char c =  char_to_byte(dat[x])<<4 | char_to_byte(dat[x+1]);
	  str += c;
  }
  return(str);
}
String AIS_NB_BC95_IOT2020:: str2HexStr(String strin)
{
  int lenuse = strin.length();
  char charBuf[lenuse*2-1];
  char strBuf[lenuse*2-1];
  String strout = "";
  strin.toCharArray(charBuf,lenuse*2) ;
  for (int i = 0; i < lenuse; i++)
  {
    sprintf(strBuf, "%02X", charBuf[i]);

    if (String(strBuf) != "00" )
    {
           strout += strBuf;
    }
  }

  return strout;
}

char AIS_NB_BC95_IOT2020:: char_to_byte(char c)
{
	if((c>='0')&&(c<='9'))
	{
		return(c-0x30);
	}
	if((c>='A')&&(c<='F'))
	{
		return(c-55);
	}
}

void AIS_NB_BC95_IOT2020:: receive_UDP(UDPReceive rx)
{
  String dataStr;
  Serial.println("################################################################");
  Serial.println("# Incoming Data");
  Serial.println("# IP--> " + rx.ip_address);
  Serial.println("# Port--> " + String(rx.port));
  Serial.println("# Length--> " + String(rx.length));
  if(sendMode == MODE_STRING_HEX)
  {
		Serial.println("# Data--> " + rx.data);
  }
  else
  {
		dataStr = toString(rx.data);
		Serial.println("# Data--> " + dataStr);
  }
  Serial.println("# Remaining length--> " + String(rx.remaining_length));
  Serial.println("################################################################");
}
